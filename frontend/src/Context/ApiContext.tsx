...
export function ApiProvider({ children }: { children: ReactNode }) {
  ...
  const completeSale = async (id: number, dispatchStatusId: number) => { const result = await api.updateSoldOrder(id, { dispatchStatusId }); setSoldOrders((items) => upsertEntity(items ?? [], result.item)); const session = await api.me(); setUser(session.user); };

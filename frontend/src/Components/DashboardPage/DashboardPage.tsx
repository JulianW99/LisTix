...
export function DashboardPage() {
  ...
  return <div className="page-stack dashboard-page">
    ...
    <section className="panel page-panel dashboard-chart"><div className="page-header"><div><h2>Monthly sales</h2><p>Payout volume and average return by month.</p></div></div><div className="chart-bars">{trend.map((item) => <div className="chart-column" key={item.label}><div className="chart-value">{formatCurrency(item.sales)}</div><div className="chart-track"><span className="chart-bar" tabIndex={0} aria-label={`${item.label}: ${formatCurrency(item.profit)} profit`} style={{ height: `${Math.max(4, item.sales / maximum * 100)}%` }}><span className="chart-profit-tooltip">Profit <strong>{formatCurrency(item.profit)}</strong></span></span></div><strong>{item.label.slice(0, 3)}</strong><small>{item.averageRoi.toFixed(1)}% ROI</small></div>)}</div></section>

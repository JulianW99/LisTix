...
export function ListingsPage() {
  const { user, tickets: cachedTickets, ticketOptions: options, loadTickets, loadTicketOptions, updateTicket, deleteTicket } = useApi();
  ...
  const canCreate = hasPermission(user, "listings.create"); const canEdit = hasPermission(user, "listings.edit"); const canDelete = hasPermission(user, "listings.delete");
  const showEditor = Boolean(user?.account?.multiUserEnabled);
  ...
  const columns: DataTableColumn<TicketItem>[] = [
    ...
    { key: "status", header: "Status", render: (ticket) => <span className="listing-marketplace-status"><StatusBadge status={ticket.marketplaceStatus} /><small>{ticket.marketplaceAvailability.filter((item) => item.enabled).length}/{ticket.marketplaceAvailability.length} marketplaces enabled</small></span> },
    ...(showEditor ? [{ key: "edited", header: "Last edited", render: (ticket: TicketItem) => <span className="listing-editor"><strong>{ticket.lastEditedBy || "System"}</strong><small>{ticket.lastEditedAt ? formatDate(ticket.lastEditedAt) : "Not edited yet"}</small></span> }] : []),
    { key: "actions", header: <span className="visually-hidden">Actions</span>, className: "listing-actions-column", render: (ticket) => (canEdit || canDelete || canCreate) ? <button className="listing-menu-button" type="button" aria-label={`Actions for ${ticket.ticketCode}`} aria-haspopup="menu" aria-expanded={menu?.ticket.databaseId === ticket.databaseId} disabled={busyId === ticket.databaseId} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => openMenu(event, ticket)}>⋯</button> : null },
  ];
  return <div className="page-stack"><section className="panel page-panel"><div className="page-header"><div><h2>Inventory listings</h2><p>Manage ticket inventory before marketplace distribution.</p></div>{canCreate && <button className="primary-button" type="button" onClick={() => navigate("/listings/new")}>Create listing</button>}</div>{(message || routeMessage) && <p className="success-message listing-feedback">{message || routeMessage}</p>}{error && <p className="error-message listing-feedback">{error}</p>}<div className="listings-table"><DataTable columns={columns} rows={listings} rowKey={(ticket) => ticket.databaseId} emptyMessage="No active listings." /></div>{menu && <div className="listing-action-menu" role="menu" aria-label={`Actions for ${menu.ticket.ticketCode}`} style={{ top: menu.top, left: menu.left }} onPointerDown={(event) => event.stopPropagation()}>{canEdit && <button type="button" role="menuitem" onClick={() => navigate(`/listings/${menu.ticket.databaseId}/edit`)}>Edit</button>}{canEdit && <button type="button" role="menuitem" onClick={() => void changeStatus(menu.ticket)}>{menu.ticket.marketplaceStatus === "Draft" ? "Activate" : "Draft"}</button>}{canCreate && <button type="button" role="menuitem" onClick={() => navigate(`/listings/new/${menu.ticket.eventId}?duplicate=${menu.ticket.databaseId}`)}>Duplicate</button>}{canDelete && <button className="delete-action" type="button" role="menuitem" onClick={() => { setDeleteTarget(menu.ticket); setMenu(null); }}>Delete</button>}</div>}</section>{deleteTarget && <Modal title="Delete listing?" onClose={() => setDeleteTarget(null)}><p className="muted">Are you sure you want to delete <strong>{deleteTarget.ticketCode}</strong>? This action cannot be undone.</p><div className="modal-actions"><button className="secondary-button" type="button" onClick={() => setDeleteTarget(null)}>Cancel</button><button className="danger-button" type="button" disabled={busyId === deleteTarget.databaseId} onClick={() => void confirmDelete()}>{busyId === deleteTarget.databaseId ? "Deleting..." : "Delete listing"}</button></div></Modal>}</div>;
}

...
export function SaleDetailsContent({ order, showActor = false }: { order: SoldOrder; showActor?: boolean }) {
  return <div className="sale-details-content">
    ...
    <section className="sale-points-card"><h3>Delivery points</h3><p>{order.pointOutcome ? <>This sale earned <strong className={order.pointOutcome.points < 0 ? "points-negative" : "points-positive"}>{order.pointOutcome.points > 0 ? "+" : ""}{order.pointOutcome.points} points</strong>.</> : <>Sending now would earn <strong className={(order.pointsIfSentNow ?? 0) < 0 ? "points-negative" : "points-positive"}>{(order.pointsIfSentNow ?? 0) > 0 ? "+" : ""}{order.pointsIfSentNow ?? 0} points</strong>.</>}</p><div className="sale-point-timeline">{order.pointSchedule.map((entry, index) => <span key={`${entry.label}-${index}`}><b className={entry.points < 0 ? "points-negative" : "points-positive"}>{entry.points > 0 ? "+" : ""}{entry.points}</b><small>{entry.label}{entry.cutoffAt ? ` · cutoff ${formatDate(entry.cutoffAt)}` : ""}</small></span>)}</div></section>
  </div>;
}

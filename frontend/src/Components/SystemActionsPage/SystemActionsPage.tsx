...
const titleForType = (type: string) => ({ sale: "New sale", retransfer: "Re-transfer", delivery_deadline_passed: "Delivery deadline passed", retransfer_unmatched: "Unmatched re-transfer" }[type] ?? type.replace(/_/g, " "));
const notificationsFor = (action: PlatformAction) => (action.details.notifications ?? {}) as { email?: NotificationResult; discord?: NotificationResult };

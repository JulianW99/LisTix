  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav" aria-label="Primary navigation">
        {settingsMode ? settingsNavigation.map((item) => <a key={item.id} href={`#${item.id}`} className="sidebar-link">{item.label}</a>) : navigation.filter((item) => hasPermission(user, item.permission)).map((item) => <NavLink key={item.to} to={item.to} className={({ isActive }) => isActive ? "sidebar-link active" : "sidebar-link"}>{item.label}</NavLink>)}
      </nav>
